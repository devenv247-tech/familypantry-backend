...
3. Generate coachSummary with Claude Haiku. System prompt must follow the CLAUDE.md raw-JSON pattern. User prompt contains ONLY the computed metrics JSON and verdict — instruct the model it may not introduce any numbers not present in the input, 3 sentences max, warm but direct tone, address the member by name. Parse, store in WeeklyAudit.coachSummary. On parse failure store a deterministic fallback string built from the verdict.
